...
import carsharing.app.service.impl.PaymentServiceImpl;
import carsharing.app.service.telegram.MessageDispatchService;
...
class PaymentServiceTest {
    ...
    void createSession_shouldCreatePayment() {
        ...
        try (MockedStatic<Session> mockedStatic = mockStatic(Session.class)) {
            ...
            Payment payment = paymentRepository.findBySessionId("session-id").orElseThrow();
            assertThat(payment.getStatus()).isEqualTo(Payment.Status.PENDING);
            assertThat(payment.getType()).isEqualTo(Payment.Type.PAYMENT);
    ...
    void getPaymentById_shouldReturnDto() {
        Payment payment = createPayment("id-001", Payment.Status.PENDING);
    ...
    void paymentCancel_shouldIgnoreIfNotPending() {
        createPayment("not-pending", Payment.Status.PAID);
    ...
    void getAllPayments_shouldReturnAllForManager() {
        ...
        final Payment firstPayment = new Payment();
        firstPayment.setAmount(BigDecimal.valueOf(100));
        firstPayment.setStatus(Payment.Status.PENDING);
        firstPayment.setType(Payment.Type.PAYMENT);
        firstPayment.setRental(rental);
        firstPayment.setSessionId("sess_1234567890");
        firstPayment.setSessionUrl("http://session.url/1");
        paymentRepository.save(firstPayment);

        final Payment secondPayment = new Payment();
        secondPayment.setAmount(BigDecimal.valueOf(200));
        secondPayment.setStatus(Payment.Status.PENDING);
        secondPayment.setType(Payment.Type.PAYMENT);
        secondPayment.setRental(rental);
        secondPayment.setSessionId("sess_0987654321");
        secondPayment.setSessionUrl("http://session.url/2");
        ...
        assertTrue(result.getContent().stream()
                .anyMatch(p -> p.amount().compareTo(firstPayment.getAmount()) == 0));

        assertTrue(result.getContent().stream()
                .anyMatch(p -> p.amount().compareTo(secondPayment.getAmount()) == 0));
    }
